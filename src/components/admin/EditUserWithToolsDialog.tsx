...
interface EditUserWithToolsDialogProps {
  open: boolean;
...
export const EditUserWithToolsDialog: React.FC<EditUserWithToolsDialogProps> = ({
  open,
  onOpenChange,
  user,
  onUpdateUser,
  ...
  const handleRemove = async () => {
    ...
    try {
      await onRemoveUser(user.id, userName);
      onOpenChange(false);
    } catch (error) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Update user profile and manage tool access for {user?.firstName} {user?.lastName}
          </DialogDescription>
        </DialogHeader>
          ...
          <TabsContent value="profile" className="space-y-4 mt-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Email is managed by Google authentication and cannot be changed here.
              </p>
            </div>
                        ...
                        <div className="flex items-center gap-4">
                          {tool.hasAccess && (
                            <Select
                              ...
                              disabled={updatingTool === tool.id || tool.source === 'company'}
                            >
                          ...
                          <Switch
                            ...
                            disabled={updatingTool === tool.id || tool.status !== 'active'}
                          />
                        </div>
                      </div>
